...
- Continued the formatter-hardening pass across
  ...
  and related exchange helpers so newer Crypto.com Exchange exports,
  malformed edge cases, and explicit pricing inputs behave more
  predictably under the shared formatter contract.
- Expanded direct regression coverage around fee modeling, malformed
  explicit pricing inputs, formatter schema guarantees, and
  representative exchange outputs, including a guard for newer CDC
  Exchange all-transactions exports that previously could produce
  orphaned fee rows with missing core fields.
- Continued the broader fiscal-audit and deterministic-pricing cleanup
  by tightening correctness coverage around ACB/superficial-loss edge
  cases and pushing more formatter/reporting paths onto the explicit
  offline pricing workflow.
...
- Consolidated more formatter end-of-pipeline behavior onto the shared
  ...
  behavior, including the dedicated raw-input pricing path in CoinSmart
  and the sparse-output normalization used by Adalite.
...
- Fixed a superficial-loss carry bug in
  ...
  substituted property in the pool and a later buy happened before the
  end of the superficial-loss window.
- Continued the recent large-history performance work by trimming
  another layer of superficial-loss overhead and avoiding unnecessary
  progress-bar setup when `verbose = FALSE`, leaving the next meaningful
  ACB bottleneck in the row-wise core mutation path rather than the
  superficial-loss prep.
- Strengthened user-facing fiscal scope documentation in the ACB
  vignette and README so the package’s conservative “identical property”
  policy is clearer for wrapped, bridged, liquid-staked, and
  exchange-specific crypto variants.

## cryptoTax 0.1.0.27

**Improvements:**

- Continued the formatter cleanup pass by introducing shared internal
  helpers for common wallet/exchange formatting seams, including
  fee-amount-to-sell-row conversion, canonical exchange-output
  finalization, and the standard resolve-prices-plus-fill-missing-totals
  path.
- Reused those new helpers across multiple formatter families, including
  Adalite, Exodus, CDC wallet, Uphold, Celsius, Presearch, PoolTool, and
  Newton, reducing repeated formatter glue while preserving current
  output contracts and empty-data behavior.
- Added direct helper coverage for the new formatter seams and kept the
  broader formatter, pricing, and ACB regression suite fully green after
  the refactor.
...
- Continued the accounting-hardening pass across
  ...
  and
  [`format_ACB()`](https://cryptoltruist.github.io/cryptoTax/reference/format_ACB.md)
  by making fee handling more explicit: Coinbase convert buys are now
  treated as fee-inclusive acquisitions, while representative exchange
  and wallet paths now have direct regressions documenting when fees
  stay separate or are modeled as separate disposition rows instead.
...
- Mirrored those new rule checks at the package level in
  ...
  replacement-share and gain-versus-loss boundaries as the lower-level
  [`ACB()`](https://cryptoltruist.github.io/cryptoTax/reference/ACB.md)
  tests.
