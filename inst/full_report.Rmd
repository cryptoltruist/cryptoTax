...
x <- report.info$report.summary[-1,]

tax_table(x)

if ("value.today" %in% x$Type && !is.null(report.info$current.price.date)) {
  warning("Date of value.today: ", report.info$current.price.date)  
}
```

```{r values, echo=FALSE, eval=FALSE}
...
value <- round(((format_dollars(report.info$report.summary$Amount[6], "numeric") + 
          ...
          last(report.info$table.revenues$total)) / 
...
<!-- All time up in percentage (including all unrealized gains and revenues received) is `r #paste0(value, "%")`. In absolute dollars, that is `r #paste0(value2 + last(report.info$table.revenues$total))` CAD. -->
...
if ("value.today" %in% names(sub2) && !is.null(report.info$current.price.date)) {
  warning("Date of value.today: ", report.info$current.price.date)  
}
```
...
report.info$table.revenues %>% 
  mutate(date.last = lubridate::date(.data$date.last)) %>% 
  select(-"currency") %>% 
  rename("total" = "total.revenues") %>% 
  tax_table(repeat.header = TRUE)
```
