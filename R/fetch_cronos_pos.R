...
#' @export
#' @examples
#' fetch_cronos_pos(limit = 10, perform = FALSE,
...
fetch_cronos_pos <- function(address, limit = 100, perform = TRUE, verbose = TRUE) {
  ...
  if (perform) {
    ...
    if (resp$pagination$total_record > limit) {
      warning("Total number of transactions detected higher than the set ",
