...
> *Note*: When you have a lot of coins, it can be better to split the calls to coinmarketcap in two or more and row bind the results with `rbind()` because otherwise we risk getting an error from the coinmarketcap API.
...
formatted.data <- lapply(exchanges, format_detect)
formatted.data <- merge_exchanges(formatted.data)
```

# Adjusted Cost Base

Next, we can begin processing that data. We start by formatting the [Adjusted Cost Base (ACB)](articles/ACB.html) of each transaction. The `formatted.ACB` is going to be our core object with which we will be working for all future steps.

```{r format_ACB}
formatted.ACB <- format_ACB(formatted.data)
```

We get a warning that there are negative values in some places, thefore that we might have forgotten some transactions. The `check_missing_transactions()` function makes it easy to identify which transactions (and therefore which coin) are concerned with this.
...
```{r report prep, echo=FALSE, message=FALSE}
library(dplyr)
tax.year <- "2021"
name <- "Mr. Cryptoltruist"
...
```{r print_report child, child = "../inst/full_report.Rmd."}
