...
- list.prices:
  ...
  `date2`. For mixed exchange workflows, you can pass one shared
  `list.prices` object here and let the relevant exchange formatters
...
- USD2CAD.table:
  ...
  contain at least `date` and `USD`. As with `list.prices`, this can be
  supplied once for a mixed workflow and will only be used by the
